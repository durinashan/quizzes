	...
	function add($vars) {
      ...
      $email_user = empty($vars['email_user']) ? 0 : 1;
		...
		$result = $wpdb->query($wpdb->prepare("INSERT INTO ".CHAINED_QUIZZES." SET
			title=%s, output=%s, email_admin=%d, email_user=%d, require_login=%d, times_to_take=%d, save_source_url=%d", 
			$vars['title'], $vars['output'], $email_admin, $email_user, $require_login, $times_to_take, $save_source_url));
	...
	function save($vars, $id) {
      ...
      $email_user = empty($vars['email_user']) ? 0 : 1;
		...
		$result = $wpdb->query($wpdb->prepare("UPDATE ".CHAINED_QUIZZES." SET
			title=%s, output=%s, email_admin=%d, email_user=%d, require_login=%d, times_to_take=%d, save_source_url=%d WHERE id=%d", 
			$vars['title'], $vars['output'], $email_admin, $email_user, 
			$require_login, $times_to_take, $save_source_url, $id));
