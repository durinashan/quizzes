	...
	static function manage() {
		...
		if(!empty($_GET['date'])) {
		   $_GET['date'] = sanitize_text_field($_GET['date']);
			switch($_GET['datef']) {
				case 'after': $filters[]=$wpdb->prepare(" DATE(datetime) > %s ", $_GET['date']); break;
				case 'before': $filters[]=$wpdb->prepare(" DATE(datetime) < %s ", $_GET['date']); break;
				case 'equals':
				default: $filters[]=$wpdb->prepare(" DATE(datetime)=%s ", $_GET['date']); break;
			}
		}
